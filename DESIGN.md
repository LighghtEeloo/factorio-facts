...
- Installed: installed layout snapshots appear as composite recipes that can be added to the focused layout, inspected through their first boundary item in the Recipes view, and unloaded only while no layout or installed snapshot references them.
...
Composite recipes are immutable snapshots, not live references to editable layouts. Installing a layout as a recipe moves that snapshot out of the editable Layouts section and into the Installed section. Unloading is guarded: a composite recipe can only be unloaded while no ordinary layout and no other installed snapshot contains a recipe entry for its `composite:` recipe id. This keeps layout graphs shareable and prevents downstream plans from changing when an upstream planning sketch is revised.

Composite recipe production size, selected machine, module/beacon intent, time, and quantities remain deliberately non-specific until the app grows real ratio solving. Composite rows leave time, machine, module, and beacon controls blank, while still allowing a production multiplier like other layout entries. Composite recipe icons are inferred from the first one to four external outputs and rendered as a fitted multi-icon stack. The layout editor header lets the user override that icon by selecting one to four external output icons and reordering them; invalid or stale overrides fall back to the inferred output order.
