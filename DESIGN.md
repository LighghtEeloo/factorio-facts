...
Each layout is an ordered list of recipe instances, not a set of recipe ids. The same recipe can appear multiple times to represent multiple copies of the same factory step. Each recipe instance has a positive production size number, an optional selected producing machine from that recipe's producer list, and optional machine-module and beacon settings. Those values are saved with the layout and shown in the layout editor as factory intent, but they are not yet used for ratio solving. Recipe card add buttons always append another instance to the focused layout. Layout rows are focused and reordered from the global sidebar, and layout recipe row drag handles reorder recipes within the focused layout. When the recipe already appears in the focused layout, the button keeps working but shows a duplicate-count hint.

Layout recipe rows should stay task-focused: show recipe identity, craft time, selected machine, compact module/beacon summaries, production multiplier, remove, and reorder controls, but avoid category pills because categories are scan noise in this editor. Production size should be presented as a multiplier with `×`, not the word `size`, so it reads as "this many copies" rather than a named recipe property.

The global layout rows let users focus, reorder, and graph layouts from anywhere in the workbench. The layout view is a focused-layout document editor: rename the current layout, reorder recipe instances, edit production sizes, selected machines, modules, and beacons, import into an empty layout, delete an empty layout, and open the focused layout's graph. Selecting a recipe row opens an in-place inspector for that recipe instance with ingredients, results, metadata, factory-intent controls, and an explicit action for opening the recipe context in the Recipes view. Double-clicking a layout recipe's identity opens that same recipe context in the Recipes view, while single-clicking keeps the user in the layout editor. Module and beacon editors should start each settings row with an icon dropdown, followed by the count, so the editor stays compact and visually aligned with recipe cards. Counts may be cleared while editing and `0` is a valid saved value, which lets users keep a configured module or beacon row temporarily disabled without losing the chosen item. The compact module/beacon summary in a recipe row is also an inspector entry point: clicking it selects that recipe and moves browser focus to the inspector so factory settings can be edited without hunting for the panel. Row selection itself should not switch app views. The import action fills an empty layout from a pasted factorio-facts layout JSON string.

The layout graph action row includes export. Export opens a readable factorio-facts layout JSON snapshot string for the open layout, with a copy action for moving it elsewhere. The snapshot includes recipe instances, production sizes, selected machines, module and beacon settings, relays, graph geometry, edge material choices, terminal choices, and layout metadata needed to reconstruct the layout through the empty-layout string import action. It is a layout interchange format, not a Factorio prototype export.

Layout state is serialized into the URL once it differs from the default empty layout. Simple app state such as the selected item, active view mode, and filters remains readable as ordinary query params. Layout-heavy state is written as a compact `s=v1.<blob>` query param: recipe ids and material keys are dictionary-encoded, recipe entries come first in the graph node index space, non-default production sizes, selected machine ids, and module/beacon settings are stored beside their recipe entries, relay nodes follow them, side choices use small numeric codes, and the compact JSON is compressed with `lz-string`'s URL-safe codec. Old `layouts=` JSON links are still accepted and migrate to the compact `s=` format on the next URL update.
...
Composite recipe production size, selected machine, module/beacon intent, time, and quantities should remain deliberately non-specific until the app grows real ratio solving. The first implementation should preserve enough boundary, machine, and production-size metadata to explain intent without claiming exact throughput.
