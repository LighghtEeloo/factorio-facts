...
The global layout rows let users focus, reorder, and graph layouts from anywhere in the workbench. The Layouts section header can import a pasted factorio-facts layout JSON string as a new focused layout, while the empty-layout editor first offers item selection through the shared picker and then a secondary import shortcut that fills that empty layout. The layout view is a focused-layout document editor: rename the current layout, review its graph-derived composite recipe boundary, reorder recipe instances, edit production sizes, export populated layouts, open the focused layout's graph, install a populated layout as a composite recipe snapshot, reset graph state through a disabled-when-empty two-step `Reset graph` header action, and delete the focused layout through a two-step confirmation at the far right of the header. The composite recipe boundary renders as ordinary scrollable content before the recipe rows, not as a pinned or floating region. Boundary rows show material identity without amount numbers. Clicking a boundary input/output material clears the focused recipe row and turns the sidebar into item context: Open in Recipes, local recipe-flag filters, and the related `Made by` or `Used in` column are shown. Installed layouts are composite recipes, but clicking one opens this same layout editor surface as an immutable read-only snapshot instead of jumping to a boundary product in the Recipes view. Installed rows and installed read-only layout headers can also open the same graph surface in immutable read-only mode. Unnamed layouts use an inferred display name when exactly one visible composite icon maps to one external output product; otherwise they fall back to the neutral untitled label. Selecting a recipe row opens an in-place inspector for that recipe instance with ingredients, results, recipe metadata, and an explicit action for opening the recipe context in the Recipes view. Clicking an inspector input material selects that material and shows the shared recipe-browser `Made by` column under the Open in Recipes action; clicking an output material shows the shared `Used in` column there. Those embedded columns keep the recipe-card add button, so users can add related recipes to the focused layout without leaving the inspector; adding a related recipe selects the newly added row. Unlike the main recipe browser, item chips inside those embedded columns are passive and do not navigate away from the layout editor. The embedded columns also expose local recipe-flag filters for locked, mining, recycling, and technology recipes below the Open in Recipes action. Double-clicking a layout recipe's identity opens that same recipe context in the Recipes view, while single-clicking keeps the user in the layout editor. Row selection itself should not switch app views.
...
The focused external input/output terminal toolbar creates a relay from that boundary set. The map terminal itself only focuses the terminal set and exposes side-docking targets. Creating this relay is conservative: the app writes the single direct edge that moves the boundary to the relay, and saves empty overrides for every other compatible edge touching the new relay. For an external input terminal, it creates `relay -> node`, hides other producer-to-relay and relay-to-consumer candidates, and lets the relay expose the required external input until the user explicitly links it. For an external output terminal, it creates `node -> relay`, hides other producer-to-relay and relay-to-consumer candidates, and lets the relay expose the required external output until the user explicitly links it. If the original terminal was optional rather than required, the selected optional material choices are removed from the original node so the boundary visibly moves to the relay.
...
Smart link is deliberately separate from recipe addition and relay creation, including relays created from external terminals. Pressing it on a focused recipe writes edge material overrides for every compatible producer or consumer around that recipe. Pressing it on a focused relay writes outgoing edge material overrides for every current graph node that can consume one of the relay's carried materials. This recovers automatic linking on demand while keeping newly added recipes and newly created relays conservative by default. Its hover preview must use the same compatibility rules as the applied command so the preview and committed graph never disagree.
