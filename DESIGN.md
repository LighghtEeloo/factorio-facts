...
The layout view lets users create, focus, rename, reorder, and graph layouts. Empty layouts show import and delete actions. The import action fills that empty layout from a factorio-facts layout JSON export.

The layout graph action row includes export. Export downloads a readable factorio-facts layout JSON snapshot for the open layout. The snapshot includes recipe instances, production sizes, relays, graph geometry, edge material choices, terminal choices, and layout metadata needed to reconstruct the layout through the empty-layout import action. It is a layout interchange format, not a Factorio prototype export.
